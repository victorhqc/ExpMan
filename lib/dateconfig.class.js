(function(){
	...
	Config.prototype.getData = function() {
		var r = {};
		r.type = this.selector.options[this.selector.selectedIndex].value;
	...
	Config.prototype.buildSelector = function() {
		...
		selector.onchange = function(e){ 
			...
			if(s._t.validateRange()){
				s._t.choosed(this, s._t.getData());
			}
		}
	...
	Config.prototype.buildDateRange = function() {
		...
		for(var i = 0, len = dates.length; i < len; i++){
			...
			d._t = this;
			d.onblur = function(){
				if(this._t.validateRange()){
					this._t.choosed(this._t, this._t.getData());
				}
			}
		}
	...
	Config.prototype.validateRange = function() {
		var valid = false;
		if(typeof this.date1 !== 'object'){
			return true;
		}

		var date1 = this.dateStringToObj(this.date1.value);
		var date2 = this.dateStringToObj(this.date2.value);
	...
	Config.prototype.dateStringToObj = function(date) {
		if(typeof date === 'object'){
			date = date.match(/(\d+)/gi);
